...
public class RingService {
	...
	//获取随机网元的ID集合
	public static List<Integer> findRandomLeftNeIdFromAllRing(List<List<Integer>> list){
		List<Integer> List=new ArrayList<Integer>();
		for(int i=0;i<list.size();i++){
			int j=list.get(i).size();
//			System.out.println("当前行环的长度"+j);
			int z = NewFunctionService.Random(3, j);
//			System.out.println("当前随机靠近中间的网元是第几个:"+z);
			int neId = list.get(i).get(z-1);
			List.add(neId);
		}		
		return List;		
	}
	...
	//获取随机网元的ID集合
	public static List<Integer> findRandomNeIdFromAllRing(List<List<Integer>> list){
		List<Integer> List=new ArrayList<Integer>();
		for(int i=0;i<list.size();i++){
			int j=list.get(i).size();
//			System.out.println("当前行环的长度"+j);
...
//			System.out.println("当前随机靠近中间的网元是第几个:"+z);
	...
	//获取随机网元的ID集合
	public static List<Integer> findRandomRightNeIdFromAllRing(List<List<Integer>> list){
		List<Integer> List=new ArrayList<Integer>();
		for(int i=0;i<list.size();i++){
			int j=list.get(i).size();
			System.out.println("当前行环的长度"+j);
