...
//			System.out.println("------------------------------分割线------------------------------------------");
			...
			if(Clist.size()==0){
				...
				if(clist.size()==0){ //汇聚旧板卡上没有空闲端口,那就用旧板卡
					  //旧板卡没有新的空闲端口就用旧的端口
					  CnewPortName = CBoard.get(1).getPortName();
					  System.out.println("汇聚旧板卡的旧端口名字为:CnewPortName:"+CnewPortName);			
					...
					CnewPortName = Cmap.get((double)OtherNewCBoard).get(0);
