...
import cn.edu.hbut.hfcrt.service.NewFunctionService;
...
public class RingController {
	...
	//获取随机三个网元中最靠近中间一个网元的上一个网元的ID集合
	public static List<Integer> findRandomLeftNeIdFromAllRingInfo(String Excel,int rowNum) throws IOException{
		List<List<Integer>> List =findRingNeIdByAllRing(Excel, rowNum);
		List<Integer> list  = RingService.findRandomLeftNeIdFromAllRing(List);		
		return list;		
	}
	
	//获取随机三个网元中最靠近中间一个网元的ID集合
	public static List<Integer> findRandomNeIdFromAllRingInfo(String Excel,int rowNum) throws IOException{
		List<List<Integer>> List =findRingNeIdByAllRing(Excel, rowNum);
		List<Integer> list  = RingService.findRandomNeIdFromAllRing(List);		
		return list;		
	}
	
	//获取随机三个网元中最靠近中间一个网元的下一个网元的ID集合
	public static List<Integer> findRandomRightNeIdFromAllRingInfo(String Excel,int rowNum) throws IOException{
		List<List<Integer>> List =findRingNeIdByAllRing(Excel, rowNum);
		List<Integer> list  = RingService.findRandomRightNeIdFromAllRing(List);		
		return list;		
	}
	...
	//获取tunnel的label
	public static List<Integer> findLabelBytunnelId(List<Integer> list){
		List<Integer> List=new ArrayList<Integer>();
		for(int i=0;i<list.size();i++){
			int tunnelId  = list.get(i);
			int label = PtnService.findTunnelLabelByTunnelId(tunnelId);
			List.add(label);
		}		
		return List;		
	}
