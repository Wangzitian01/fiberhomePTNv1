...
import cn.edu.hbut.hfcrt.service.BusinessListServcie;
...
//		MainDemo2.main01();
		...
		System.out.println(pathList.get(1));
 
		for(int i=0;i<pathList.size();i++){
			if(i==0){
				List<BusinessList> rBSL= RingRightTest(path);
				List<BusinessList> lBSL= RingLeftTest(path);
				List<OldCircle> OldCircleList = ReadExcel.ReadOldCircleFromExcel(ExcelPath, rowNum);
				System.out.println("hahahahha:"+rBSL.size());
				System.out.println(rBSL);
				ReadExcel.writeToatlListToExcel(rBSL, "C:\\Users\\13409\\Desktop\\列表清单总.xls",OldCircleList,lBSL);
			}
			else{
				RingRightTest(pathList.get(i));//==>
				RingLeftTest(pathList.get(i)); //<==
			}
		}		 
	}
		
	public static List<BusinessList> RingRightTest(String path) throws IOException{
		List<BusinessList> BSL=new ArrayList<BusinessList>();
		for(int i=0;i<=n;i++){
			System.out.println("当前行数是第==========================================="+i+"行=======================================");
            
			//随机产生,方案三
			List<Integer> lList1= RingController.findRandomNeIdFromAllRingInfo(ExcelPath, rowNum); //随机三次,每次产生三个点,找距离中间最短的
			System.out.println("第"+i+"条的中间随机产生的网元id为:"+lList1.get(i));
			List<Integer> rList1= RingController.findRandomRightNeIdFromAllRingInfo(ExcelPath, rowNum);
//			System.out.println("第"+i+"条的中间随机产生的网元下一个网元id为:"+rList1.get(i));	
...
//			System.out.println("中间左网元ID"+neId1);
			...
			List<Document> tunnelList = PtnService.selectTunnels2(NeId1, path);
...
//			List<Document> tunnelList = PtnService.selectTunnels1(boardId1, portName1, neId1, path);
			...
			if(tunnelList.size()==0){
				System.out.println("初始筛选的tunnel的条数为0");
				BusinessList BS =new BusinessList();
				BS.setTunnelId("0");
				
				BS.setMidNeName("0");
				BS.setConvergenceNeName("0");
				BS.setGBoardId(0.0);
				BS.setGNewPortName("0");
				BS.setGslotAddress(0);
				
				BS.setCBoardId(0.0);
				BS.setCNewPortName("0");
				BS.setCslotAddress(0);
				BS.setGNeName_SlotNo_PortNo("0");
				BS.setCNeName_SlotNo_PortNo("0");
				BS.setLabel("0");
				BSL.add(BS);
				continue;
			}
...
//			System.out.println("二次筛选后汇聚网元集合情况"+CList1);
			...
			//新增	========================
			BoardPort GBP = BList.get(0);  // 0是目标网元     
		    Double GboardId = (double) GBP.getNewBoard();
		    String GPortName = GBP.getNewPortName();
		    
		    BoardPort CBP = BList.get(1);  // 1是汇聚网元
		    Double CboardId = (double) CBP.getNewBoard();
		    String CPortName = CBP.getNewPortName();
		    
			 		
			List<Integer> tunnelIdList = ChangeTunnelService.findTunnelId(tunnelList1);
			BusinessList BS = BusinessListServcie.AddClass(tunnelIdList, (int)NeId1, FConvergenceNeId2, GboardId, GPortName,CboardId,CPortName);
...
//			ReadExcel.writeListToExcel(map1,"C:\\Users\\13409\\Desktop\\烽火\\tunnel变化后结果.xls");
	...
	public static List<BusinessList> RingLeftTest(String path)throws IOException{
		List<BusinessList> BSL=new ArrayList<BusinessList>();
		for(int i=0;i<=n;i++){
			System.out.println("当前行数是第==========================================="+i+"行=======================================");
			//随机产生,方案三
...
//			System.out.println("中间右网元ID"+neId1);
			...
			List<Document> tunnelList = PtnService.selectTunnels2(NeId2, path);
//			List<Document> tunnelList = PtnService.selectTunnels1(boardId1, portName1, neId1, path);
			System.out.println("-----------------初始tunnel的条数---------------------"+tunnelList.size());
			if(tunnelList.size()==0){
				System.out.println("初始筛选的tunnel的条数为0");
			...
			List<Integer> GList =ChangeTunnelService.JudgeNeLocationInTunnelList((int)NeId2, tunnelList,path);
			System.out.println("目标网元在所经过的tunnel中网元的位置"+GList);
			System.out.println("-----------GList.size()的大小为------------------------"+GList.size());
...
//			System.out.println("二次筛选后汇聚网元集合情况"+CList1);
			...
			List<BoardPort> BList = TunnelService.selectNewBoardAndNewPortNameFromOneTunnel(Atunnel, (int)NeId2, AConvergenceNeId2, path);
			...
			List<Integer> tunnelIdList = ChangeTunnelService.findTunnelId(tunnelList1);
			BusinessList BS = BusinessListServcie.AddClass(tunnelIdList, (int)NeId2, AConvergenceNeId2, GboardId, GPortName,CboardId,CPortName);
			...
			//删网元以及更新板卡业务操作
			ChangeTunnelService.TunnelChange(tunnelList1, (int)NeId2, AConvergenceNeId2, path, BList);
